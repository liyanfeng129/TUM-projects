...
def affine_forward(x, w, b):
    ...
    ########################################################################
    x_reshaped = x.reshape(N, -1)  # reshapes into matrix where first dimension is N and the second dimension is
    # whatever needed, so in this case it will be D = d_1 * ... * d_k
...
def affine_backward(dout, cache):
    ...
    ########################################################################
    N = x.shape[0]
    x_reshaped = x.reshape(N, -1)  # now x has shape (N, D)
    dx_temp = np.dot(dout, w.T)
    dx = dx_temp.reshape(x.shape)  # reshape to (N, d1, ..., d_k)
