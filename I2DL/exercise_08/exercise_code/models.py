...
class Encoder(nn.Module):
    ...
    def __init__(self, hparams, input_size=28 * 28, latent_dim=20):
        ...
        # Define the encoder architecture using nn.Sequential()
        self.encoder = nn.Sequential(
            nn.Linear(self.input_size, self.hparams["n_hidden1"]),
            nn.BatchNorm1d(self.hparams["n_hidden1"]), 
            nn.ReLU(),
            nn.Dropout(p=self.hparams["drop_prob"]),
            nn.Linear(self.hparams["n_hidden1"], self.hparams["n_hidden2"]),
            nn.BatchNorm1d(self.hparams["n_hidden2"]),
            nn.ReLU(),
            nn.Dropout(p=self.hparams["drop_prob"]),
            nn.Linear(self.hparams["n_hidden2"], self.hparams["n_hidden3"]),
            nn.BatchNorm1d(self.hparams["n_hidden3"]),
            nn.ReLU(),
            nn.Dropout(p=self.hparams["drop_prob"]),
            nn.Linear(self.hparams["n_hidden3"], latent_dim),
            nn.ReLU()
        )
...
class Decoder(nn.Module):
    ...
    def __init__(self, hparams, latent_dim=20, output_size=28 * 28):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(latent_dim, self.hparams["n_hidden3"]),
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden3"], self.hparams["n_hidden2"]),
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden2"], self.hparams["n_hidden1"]),
            nn.ReLU(),
            nn.Linear(self.hparams["n_hidden1"], output_size),
            nn.Sigmoid()
...
class Classifier(nn.Module):
    ...
    def __init__(self, hparams, encoder):
        ...
        self.model = nn.Sequential(
            nn.Linear(self.encoder.latent_dim, self.hparams["n_hidden1"]),
            nn.BatchNorm1d(self.hparams["n_hidden1"]),
            nn.ReLU(),
            nn.Dropout(p=self.hparams["drop_prob"]),
            nn.Linear(self.hparams["n_hidden1"], self.hparams["n_hidden2"]),
            ...
            nn.ReLU(),
            nn.Dropout(p=self.hparams["drop_prob"]),
            nn.Linear(self.hparams["n_hidden3"], 10),
        )
