...
class HMM_TxtGenerator:
    ...
    def forwards(self, x):
        ...
        alpha : np.ndarray, shape=(T,K)
                alpha(t,k) = Pr(Z_t=k,x[1:t])
        """
        A = self.model_params.A  # [KxK], transition matrix, K is number of the states
        B = self.model_params.B  # [KxV] , observation matrix, V is number of the observations
        pi = self.model_params.pi  # [Kx1] ------, stationary state probability
        T = len(x)
        K = A.shape[0]
        alpha = np.zeros((T,K))
        ...
        alpha[0] = pi.flatten() * B[:, x[0]]  # Element-wise multiplication, x[0] represents the first observation in the   sequence x. Since x is a list of word indices, x[0] gives us the index of the first word in the sequence.
        # Forward recursion
        #for t in range(1, T):
        #   for k in range(K):
        #        alpha[t, k] = np.sum(alpha[t - 1] * A[:, k]) * B[k, x[t]]
        for t in range(1,T):
            alpha[t] = B[:, x[t]]*np.dot(A.T, alpha[t-1])
    ...
    def log_likelihood(self, alpha):
        ...
        ### YOUR CODE HERE ###
        T = alpha.shape[0]
        # Pr(X_{1:T}) = sum_k(pr(X_{1:T},Z_t = k))
        #alpha[T - 1], which represents the total probability of all possible paths through the hidden states at the last time step.
    ...
    def backwards(self, x):
        ...
        for t in range(T - 2, -1, -1):
            beta[t] = np.dot(A, B[:, x[t+1]] * beta[t+1])
        return beta
    ...
    def generate_sentence(self, sentence_length):
        ...
        sent = []
        current_state = np.random.choice(range(K), p=pi )  # Choose a random initial state
        for _ in range(sentence_length):
            # Sample the next word index based on the emission probabilities of the current state
            word_index = np.random.choice(range(V), p=B[current_state])
            sent.append(word_index)
            # Transition to the next state based on the transition probabilities
            current_state = np.random.choice(range(K), p=A[current_state])
