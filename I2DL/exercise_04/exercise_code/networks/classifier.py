...
class Classifier(Network):
    ...
    def forward(self, X):
        ...
        z = self.sigmoid(np.matmul(X, self.W))
        self.cache = (X, z)
    ...
    def backward(self, dout):
        ...
        """"
                temp = np.dot(np.transpose(X), (z-z**2))

                temp2 = np.dot(temp, np.transpose(dout))

                temp3 = np.mean(temp2, axis=1)


                dW = temp3.reshape((len(temp3), 1)) # shape frpm (D+1,) to (D+1, 1)
            """
        #dL/dW = ds/dW * dz/ds * dL/dz
