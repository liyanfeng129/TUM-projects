...
def compute_token_to_index(vocabulary: Tuple[str], counts: Tuple[int]) -> Tuple[Dict[str, int], Dict[int, str]]:
    ...
    ###########################
    # YOUR CODE HERE
    token_to_index = {token: idx for idx, token in enumerate(vocabulary)} #key is token, value is index
    index_to_token = {idx: token for idx, token in enumerate(vocabulary)} #key is index, value is token
    index_to_count = {idx: count for idx, count in enumerate(counts)}     #key is index, value is count
    ...
    for i, token in enumerate(sequence):
        ...
        for j in range(start, end):
            if i != j: #skip the word itself
                context = sequence[j]
                context_idx = token_to_index[context]
                token_pairs.append((token_index, context_idx))
